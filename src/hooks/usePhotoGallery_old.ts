...
export async function base64FromPath_(path: string): Promise<string> {
...
export function usePhotoGallery() {
      ...
      if (isBase64) {
        alert(
          "isBase64>>>> " +
            JSON.stringify({
              filepath: fileName,
              // webviewPath: await base64FromPath(photo.webPath!),
  ...
  const loadSaved = async () => {
    ...
    const photosInStorage: any = value ? JSON.parse(value) : [];
  ...
  const takePhoto = async (
    ...
    const savedFileImage = await savePicture(photo, fileName, isBase64);
  ...
  const uploadPhoto = async (photo: UserPhoto) => {
    ...
    ////// const baseURL = "https://api.taskagram.com/";
